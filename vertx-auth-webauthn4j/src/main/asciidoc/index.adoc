...
1. A call to generate a {@link io.vertx.ext.auth.webauthn4j.WebAuthn4J#createPublicKeyCredentialCreationOptions(JsonObject)}
...
1. A call to generate a {@link io.vertx.ext.auth.webauthn4j.WebAuthn4J#createPublicKeyCredentialRequestOptions(String)}
2. A call with the solution to the challenge to the normal `authenticate` API method.
