...
public class WebAuth4JNTest {
  ...
  public void testFIDOLogin(TestContext should) {
    ...
    WebAuthn4J webAuthN = WebAuthn4J.create(
        rule.vertx(),
        new WebAuthn4JOptions().setRelyingParty(new RelyingParty().setName("FIDO Examples Corporation")).setRequireResidentKey(true))
        .credentialStorage(database);
  ...
  public void testPAckedFull(TestContext should) {
    final Async test = should.async();
    WebAuthn4J webAuthN = WebAuthn4J.create(rule.vertx(), new WebAuthn4JOptions().setRelyingParty(new RelyingParty().setName("FIDO Examples Corporation")))
        .credentialStorage(database);
