...
_Authorization_ means verifying a user has an authority.
...
Once you have an {@link io.vertx.ext.auth.User} instance you can call methods on it to authorize it. A newly created
...
Here's an example loading the authorization cache an authorizing a user:
