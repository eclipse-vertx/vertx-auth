...
{@link io.vertx.ext.auth.mongo.MongoAuthenticationOptions#getUsernameCredentialField(String) } and
{@link io.vertx.ext.auth.mongo.MongoAuthenticationOptions#getPasswordCredentialField(String) }
...
{@link io.vertx.ext.auth.User#isAuthorized(io.vertx.ext.auth.authorization, io.vertx.core.Handler)} as follows:
...
If validating that a user has a particular _role_ then you simply use the {@link io.vertx.ext.auth.authorizationRoleBasedPermission}.
