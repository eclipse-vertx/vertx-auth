...
To create an instance of the provider you use `link:jsdoc/jwt_auth-JWTAuth.html[JWTAuth]`. You specify the configuration
in a JSON object.
...
[source,java]
----
var JWTAuth = require("vertx-auth-js/jwt_auth");
...
var provider = JWTAuth.create(config);
