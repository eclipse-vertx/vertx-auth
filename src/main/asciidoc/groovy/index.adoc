...
The operation `link:groovydoc/io/vertx/groovy/ext/auth/AuthProvider.html#fromBuffer(io.vertx.core.buffer.Buffer)[fromBuffer]` allows a User object
to be reconstructed from a `link:../../vertx-core/groovy/groovydoc/io/vertx/groovy/core/buffer/Buffer.html[Buffer]`. This is primarily used in vertx-web session clustering
