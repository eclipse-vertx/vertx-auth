...
<<<<<<< HEAD
with the operations {@link io.vertx.ext.auth.jdbc.JDBCAuthentication#setAuthenticationQuery(String)},
{@link io.vertx.ext.auth.jdbc.JDBCAuthorization#setPermissionsQuery(String)} and
{@link io.vertx.ext.auth.jdbc.JDBCAuthorization#setRolesQuery(String)}, if you want to use them with a different
...
THe hashing strategy is now required to instantiate JDBCAuthentication {@link {@link io.vertx.ext.auth.jdbc.JDBCAuthentication#create(JDBCClient, JDBCHashStrategy,  JDBCAuthenticationOptions)} 
You have the choice between SHA-512 or PBKDF2. Note that as of 2018-01-08 OWASP recommends
>>>>>>> Added back the class JDBCAuth to be backward compatible. Note that the whole class is marked as deprecated to encourage people to switch to JDBCAuthencation / JDBCAuthorization
...
{@link io.vertx.ext.auth.jdbc.JDBCAuthentication#setHashStrategy(JDBCHashStrategy)}.
>>>>>>> updated code based on comments from Paulo:
