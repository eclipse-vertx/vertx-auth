...
with the operations {@link io.vertx.ext.auth.jdbc.JDBCAuthenticationProvider#setAuthenticationQuery(String)},
{@link io.vertx.ext.auth.jdbc.JDBCAuthorizationProvider#setPermissionsQuery(String)} and
{@link io.vertx.ext.auth.jdbc.JDBCAuthorizationProvider#setRolesQuery(String)}, if you want to use them with a different
database schema.
...
{@link io.vertx.ext.auth.jdbc.JDBCAuthenticationProvider#setHashStrategy(JDBCHashStrategy)}.
