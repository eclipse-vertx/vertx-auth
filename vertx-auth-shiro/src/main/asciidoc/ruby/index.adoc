...
`link:../../vertx-auth-common/ruby/yardoc/VertxAuthCommon/User.html#is_permitted-instance_method[isPermitted]` as follows:

[source,java]
----

user.is_permitted("newsletter:edit:13") { |res,res_err|
  if (res_err == nil)
    hasPermission = res
  else
    # Failed to
...
user.is_permitted("role:manager") { |res,res_err|
  if (res_err == nil)
    hasRole = res
  else
    # Failed to
