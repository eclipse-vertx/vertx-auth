...
provider.authenticate(authInfo) { |authenticate,authenticate_err|
  if (authenticate_err == nil)
    authenticate.is_permitted("newsletter:edit:13") { |hasPermission,hasPermission_err|
      if (hasPermission_err == nil)
